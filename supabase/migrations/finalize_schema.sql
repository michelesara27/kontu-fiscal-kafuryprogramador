/*
# Finalização do Esquema do Banco de Dados
...
CREATE TABLE IF NOT EXISTS public.tipos_obrigacao (
  ...
  nome TEXT NOT NULL,
  descricao TEXT,
  frequencia_padrao TEXT NOT NULL,
...
-- Recriar todas as políticas de segurança
-- Profiles
...
-- Clientes
CREATE OR REPLACE POLICY "Users can create their own clients" 
ON public.clientes 
FOR INSERT 
WITH CHECK (auth.uid() = user_id);

CREATE OR REPLACE POLICY "Users can view their own clients" 
ON public.clientes 
FOR SELECT 
USING (auth.uid() = user_id);

CREATE OR REPLACE POLICY "Users can update their own clients" 
ON public.clientes 
FOR UPDATE 
USING (auth.uid() = user_id);

CREATE OR REPLACE POLICY "Users can delete their own clients" 
ON public.clientes 
FOR DELETE 
USING (auth.uid() = user_id);

-- Tipos de Obrigação (acesso público)
...
-- Obrigações
CREATE OR REPLACE POLICY "Users can create their own obligations" 
ON public.obrigacoes 
FOR INSERT 
WITH CHECK (auth.uid() = user_id);

CREATE OR REPLACE POLICY "Users can view their own obligations" 
ON public.obrigacoes 
FOR SELECT 
USING (auth.uid() = user_id);

CREATE OR REPLACE POLICY "Users can update their own obligations" 
ON public.obrigacoes 
FOR UPDATE 
USING (auth.uid() = user_id);

CREATE OR REPLACE POLICY "Users can delete their own obligations" 
ON public.obrigacoes 
FOR DELETE 
USING (auth.uid() = user_id);

-- Históricos
CREATE OR REPLACE POLICY "Users can view history of their obligations" 
...
CREATE OR REPLACE TRIGGER update_obrigacoes_updated_at
  BEFORE UPDATE ON public.obrigacoes
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_column();

CREATE OR REPLACE TRIGGER update_tipos_obrigacao_updated_at
...
CREATE OR REPLACE TRIGGER trigger_registrar_historico
